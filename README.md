...
| `bundle --model <m>` | copy this build's exe+DLLs into a model dir; run it later without rebuilding |
| `ckpt2model --checkpoint <c> --out <m>` | extract weights from a training checkpoint into a `model.bin` gen/report can load |
