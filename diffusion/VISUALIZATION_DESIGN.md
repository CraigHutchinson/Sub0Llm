...
| phase | deliverable | engine change | value |
|-------|-------------|---------------|-------|
| **A** | iteration timeline JSON + web scrubber (canvas + confidence heatmap + commit order) | extend `refine_canvas` `on_iter` to emit a Frame | watch diffusion "snap into focus"; smallest, highest ratio |
| **B** | MERA per-level internals (pyramid panel, gist proj2d, attn) | opt-in `ForwardTrace*` on `MeraDenoiser::forward` | see the tiered hierarchy think |
| **C** | prompt box + param knobs + side-by-side trace diff | `/v1/generate_trace` server endpoint | parameter-adjustment comparison; verbosity slider |
| **D** | training-time trajectory replay across checkpoints | checkpoint-tagged traces | watch it learn |
