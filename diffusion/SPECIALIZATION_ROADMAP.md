...
| **7** | end-to-end: device d2d `copy()` + `Node::accumulate_grad` (unblocks ALL the wired bwd dispatches) → `weighted_cross_entropy` on CUDA → Denoiser fwd+bwd, gradient-checked vs CPU; mini training loop | 1-2 | Yes |
