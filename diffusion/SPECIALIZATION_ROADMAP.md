...
**Phases 1 (softmax fwd) + 2 (rms_norm fwd+bwd) also DONE** — CUDA kernels + dispatch + parity
tests + CPU-vs-GPU bench rows (588 CPU / 545 core CUDA cases green). Remaining: ~4 CUDA kernels
(matmul_tb, rope fwd+bwd, silu/embedding bwd) and `GemmaGpuLayers` is inference-only. The autograd
BACKWARD-on-CUDA end-to-end path also needs device d2d `copy()`/`accumulate_grad` (Phase 7). Phases
3+ need the CUDA build (`cuda`/`cuda-native` preset — see memory `cuda-build-setup`).
...
**Recommended start:** ~~Phase 0~~ ~~Phase 1~~ ~~Phase 2~~ DONE. Next: **Phase 3 (matmul_tb —
Aᵀ·B, weight grads)**, currently `require_cpu` in `ops::matmul_tb`. Add the CUDA kernel + dispatch
+ parity test + bench row (same shape as the rms_norm work). Then Phase 4 (rope fwd+bwd). The
device d2d `copy()` / `accumulate_grad` gap (needed for autograd BACKWARD end-to-end on CUDA) is
best done as part of Phase 7 (end-to-end Denoiser) — until then, validate bwd kernels directly
(H2D→launch→D2H vs CPU) as Phase 2 does.
