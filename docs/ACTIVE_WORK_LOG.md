...
**2026-09-11 Claude Code -- B38 done, merged, pushed.** SIMD-reduce integrated into `main` as a real,
permanent, opt-in `--simd-reduce` build option (default `false`, byte-for-byte unaffected by construction
-- every dispatcher's `UseSimd=false` arm is a literal copy of the original scalar loop). Independently
reverified (dispatcher code read in full, suites match exactly at 56/2 and 120,945/246, auto-merged
`forward_row_via_run_ex` signature confirmed correct alongside B36's own prefetch hook). Resolved a real
conflict at `decode.cpp`'s MoE call site (B36's pipelined-I/O lambda + B38's `<USE_SIMD_REDUCE>` template
...
| 2026-09-22 | Sonnet 5 subagent C (isolated worktree) | O5 phase 2a: streaming native-quant unpack kernels (Q8_0/Q4_K/Q5_K/Q6_K), gated at >= 10 GB/s of compressed bytes per thread; bench fixed to compare against the real `gemv::axpy` | agent worktree | `include/sub0/backbone_quant_dot.hpp`, `tests/backbone_quant_dot_tests.cpp`, `benchmarks/backbone_quant_dot_bench.cpp`, `docs/BACKBONE_NATIVE_QUANT.md`. NO engine wiring (phase 2b waits for O4, since it touches `decode.cpp`) | done, merged | Merged 2026-09-22. Re-verified: gates exact (3,891/10; 141,609/257; 145,500/267), throughput reproduced, and the kernels are 1.21-2.17x faster per row than `gemv::axpy` at 8 threads. Phase 2b (engine wiring) NOT started and is the next real step. |
| 2026-09-22 | Sonnet 5 subagent D (isolated worktree) | O5 phase 2b-1: the native-quant backbone SIDECAR (S0B1) -- format, `sub0llm-transplant --backbone-quant` emit, loader, round-trip vs the real shards | agent worktree | `include/sub0/backbone_quant.hpp`, `tools/sub0llm-transplant.cpp`, new tests, `docs/BACKBONE_NATIVE_QUANT.md` S13 | done, merged | Integrated as `3442dbf` with loader geometry and portable verification fixes in `44dade3`; targeted sidecar suite 143 assertions/9 cases passed. Engine wiring remains open. |
| 2026-09-22 | Sonnet 5 subagent E (isolated worktree) | O5 kernel pass 4: per-256 activation block and AVX-VNNI comparison | agent worktree | `include/sub0/backbone_quant_dot.hpp`, its tests and benchmark, `docs/BACKBONE_NATIVE_QUANT.md` S14 | done, merged | Integrated as `1d0f752` after review fixes; 6,888 assertions/17 kernel cases passed and portable benchmark syntax passed. The faster super path increases activation error on real weights; end-to-end logit quality and engine wiring remain open. |
| 2026-09-22 | Codex + delegated audit agent | Intel I21 selected-arm prepared-copy resume | `main` | `benchmarks/intel/mechanisms/prepared_copy.cpp`, `scripts/intel/bench/`, Intel I21 docs/evidence; no engine or O5 files | done; comparison pending | One selected arm per process while preserving all-mode inventory. DPC++ 2025.3.3 compile, synthetic contract validator, 257-element all-mode inventory, separate prepared-arm smoke and one-pair/two-process I21 integration smoke passed. No controlled timing or promotion claim; I21 reserved alternating-process comparison remains open. |
| 2026-09-23 | Codex + delegated S1.5 agent | Intel I19 scratch/hint bounded probe | `main` | `benchmarks/intel/memory/scratch_hint.cpp`, `scripts/intel/memory/run-scratch-hint.ps1`, `docs/INTEL_IGPU_SCRATCH_HINT_SPIKE.md` | done; comparison pending | DPC++ compile and real Level Zero correctness runs at 257 and 65,536 elements passed for shared/device/shared-prefetch. Advice remains explicitly unsupported without a verified device code. No controlled performance inference. |
| 2026-09-23 | Codex + delegated I18 agent | Intel I18 IQ fixture audit | `main` | Read-only fixture admission review; no source edits | paused on fixture admission | Agent resumed and confirmed that admitted IQ plane fixtures are still missing. Kernel or throughput claims remain gated on real admitted data; no speculative implementation was merged. |
| 2026-09-23 | Codex + delegated loader/review agents | O5 phase 2b-2a opt-in native backbone decode wiring | `main` | Configurator, sidecar load/API, CPU decode, pair identity, tests and O5 docs | done | First two of 388 sidecar roles wired (untied TokEmb and Q4_K head); tied configuration rejected. Full-file pair guard blocks same-shape file mixes. Default-off neutral suite 29,510,661/147 exactly; opt-in real 48-layer logits finite, L2-relative 0.162039, argmax 5/6. Targeted 7,313/28. No throughput claim; remaining roles and controlled performance gate are separate O5 increments. |
| 2026-09-23 | Claude Code (+ Sonnet 5 subagent) | O5 phase 2b-2b: recover the GDN roles in the S0B1 sidecar (in-proj rows permuted at write; out-proj input permutation applied to the activation), then wire every remaining sidecar role into decode (GR, QSA, MoE shared expert, GDN) with caller-owned activation scratch, then the controlled throughput gate | agent worktree | `tools/sub0llm-transplant.cpp`, `include/sub0/backbone_quant.hpp`, `include/sub0/backbone_quant_dot.hpp`, `include/sub0/gdn_math.hpp`, `src/backends/cpu/decode.cpp`, tests, `docs/BACKBONE_NATIVE_QUANT.md` S16 | done, merged (GDN only; GR/QSA/MoE-shared not wired) | Sidecar recovers all 3 GDN roles (388->496 tensors, 3.261 GiB native / 2.75x vs bf16, real 48-layer gates 0 mismatches incl. a 41.9M-value full-row cross-check and a gather-direction proof) and old (388-role) sidecars still load, reproducing Codex's own 0.162039/5-6 exactly. `gdn::forward` wired for in_qkv/in_z/out_proj behind the existing `--backbone-quant-dot` flag, T==1-only, geometry re-validated, falls back to axpy per-pair when absent. Also closed the `Gsum16` TODO(phase 2b) AGENTS.md S1 heap-alloc (moved to public scope, now caller-owned/reused). Default-off exact: `sub0_tests` 29,510,661/147 (fingerprint hash unchanged), `sub0_frontend_tests` 149,177/288 (baseline+255/+3, fully accounted). Opt-in real 48-layer: forward() unaffected (byte-identical rows), forward_one L2-relative 0.24556 argmax 5/6 (combines phase 2b-2a's embed/head + this pass's 3 GDN roles across all 36 GDN layers). Throughput: REDUCED protocol only (2 rounds not 4, background load ~5.8% marginally over the 5% gate) -- opt-in ~5.9% faster median after the Gsum16 fix (pre-fix: flat/mixed), directionally positive, NOT a clean KPI-gate pass; re-run the full protocol on an idle host before citing. GR/QSA/MoE-shared-expert roles remain unwired (already in the sidecar from 2b-1, no consumer yet) -- named as the next increment, not attempted this pass. Full numbers and the gather-direction proof's algebra: `docs/BACKBONE_NATIVE_QUANT.md` S16. **Primary-agent correction at merge (2026-09-25): the throughput above is VOID** -- both arms were built without `--moe-quant-dot 1`. Decode-time first-touch faults on the mapped sidecar (551k vs 82k) also hid the GDN gain; fixed with `Store::prefault()` at load. Valid A/B at the real flags: 0.218 -> 0.202 s/token (-7.3%, 6/6 rounds), GDN -25%, lm_head -46%, other bf16 phases +5-13% (open). L2 0.2788 vs 0.2325 at those flags. See S17. |
| 2026-09-25 | Sonnet 5 subagent (isolated worktree, agent-a6ed982fca49081e4) | O5 phase 2b-3: fix the resolve-per-token seam (S17's own finding) with a resolved-role table + a storage-agnostic `bbqd::Plane`, then wire Gated Residual/QSA/MoE-shared-expert into decode, then the controlled A/B | agent worktree | `include/sub0/backbone_quant_dot.hpp`, `include/sub0/gdn_math.hpp`, `include/sub0/gated_residual_math.hpp`, `include/sub0/qsa_math.hpp`, `include/sub0/moe_math.hpp`, `src/backends/cpu/internal.hpp`, `src/backends/cpu/backend.cpp`, `src/backends/cpu/decode.cpp`, tests, `docs/BACKBONE_NATIVE_QUANT.md` S18 | active | Phase A+B committed (`fe22be5`, `4d47db9`): `BackboneRoleTable` resolves every (role,layer) once at sidecar load; `bbqd::Plane` decouples the `*_math.hpp` headers from `bbq::Desc`. GR/QSA/MoE-shared wired behind the existing default-off `--backbone-quant-dot`; MoE-shared took a `requires`-detected `compute_shared()` hook on `ParallelExperts` instead of a Native struct in moe_math.hpp, to avoid a real header cycle (moe_quant_dot.hpp depends on moe_math.hpp). No sidecar/on-disk format change -- every role wired was already in the existing 496-role `.bbq`. Neutral gates exact: `sub0_tests` 29,510,661/147, fingerprints unchanged; `sub0_frontend_tests` 208,910/293 (baseline+2,092/+1, a new QSA row-selection proof against the transposed bf16-blob-shaped weight). Real-artifact A/B in progress -- see this row's own follow-up entry. |
| 2026-09-23 | Sonnet 5 subagent (isolated worktree) | O7: routed-expert (MoE) kernel speedups -- `SuperCache` (hoist IQ1_S/IQ2_XXS's per-superblock `d` decode out of the per-32-group loop) + `_mm256_i32gather_epi64` codebook/sign lookup (pass 1+2, bit-exact, folded into the default `gemv_plane` dispatch); AVX-VNNI `vpdpbusd` evaluated as pass 3, tested, parked unwired (roughly parity-to-worse) | agent worktree | `include/sub0/moe_quant_dot.hpp`, `tests/moe_quant_tests.cpp`, `tools/sub0llm-bench-moeqd.cpp`, new `docs/optimization/opportunities/O7_expert_kernels.md` | parked, merged default-off | Kernel microbench (this host, real Qwen4 plane shapes, one thread): IQ1_S/IQ2_XXS **14-26% faster per plane** across both shapes and both hot/stream memory modes (IQ1_S down/hot the one cell inside host noise); IQ4_NL untouched by construction (`kHasSuper = false`), its own apparent movement in the raw logs confirmed to be host noise by re-running the byte-identical pre-O7 header. Bit-exact by construction and checked, not assumed: new `[moequant]` cases `REQUIRE` exact (`==`) agreement against each kernel's pre-O7 reference (kept in the tree, unmodified) over real plane shapes, plus a mutation test proving the cache genuinely gates the value rather than always refreshing. `G-HASH`/`G-SUITE-ENGINE` verified structurally unreachable from the neutral build (`MOE_QUANT_DOT` `if constexpr`-gates every call) AND empirically (neutral `sub0_tests` rebuilt+run with this package's header and, separately, with the byte-identical pre-O7 original swapped back in -- identical fingerprints and identical 29,510,661/147 both times). **One pre-existing finding, not caused by this package**: the neutral-build fingerprints reproduced here (`5a7382ea70d3913b`/`7f44bdae18c313dd`/`d1625d19ed2258f1`) do not match `docs/optimization/kpi_gates.json`'s recorded values -- a drift on `main` already present before this package touched anything, confirmed by the same before/after swap; flagged, not fixed (out of file scope). `sub0_frontend_tests` 148,922/285 -> 206,563/289 (+4 cases, all `[moequant]`). VNNI's pass-3 numbers were captured right as the primary agent's timing hold arrived -- reported as preliminary/directional only, not a final percentage; real-artifact `G-QUALITY`/`G-PARITY`/`G-PERF` deliberately NOT run (out of scope per the brief). Full write-up, reference-study quotes, and every pass's numbers: `docs/optimization/opportunities/O7_expert_kernels.md`. **Primary-agent verdict (2026-09-25): bit-exact end to end (L2 identical), but the real decode's routed-expert phase got ~4% SLOWER (44.7 -> 46.6 ms/token, 5/5 rounds). Merged parked behind `detail::kO7Kernels = false`; next passes (drop the gather, 10-thread bench, 8 expert threads) in O7_expert_kernels.md S10.** |

---

**2026-09-22 Claude Code handoff — resumed by Codex.** Session `/goal` was left
...
- **O4 (as handed back by the agent, before merge): `worktree-agent-ac92618df53668765` @ `42f82a0`.**
  - Lever 1 threads GDN's conv1d+roll (per channel), recurrence (per k-head) and RMSNormGated (per
    ...
    ms/token.
  - Lever 2: `ParallelExperts` `schedule(static)` → `schedule(dynamic)`. Agent measured routed experts
    at 38.3 → 35.2 ms/token.
  - The agent reported unchanged gates: 28,969,623/147, 141,609/257, all three fingerprints, and L2
    0.23252 at GEMV threads 1 and 8.
  - **Diff reviewed by the primary agent: the bit-exactness arguments hold.** No threaded loop reduces
    across its split axis; the b/a axpy keeps the i-ascending order; the phase-2 MoE sum stays in fixed
    k order.
  - **Left before merge:**
    1. merge `--no-ff` onto current main (no file overlap with O5);
    2. rebuild `d196check` and run both suites. Frontend is now **144,457/263** on main, not 141,609;
    3. run the fingerprints, and the real-artifact L2 at the recommended flags;
    4. an interleaved A/B at low contention;
    5. write `docs/optimization/opportunities/O4_*.md`, add its README index row and a perf_history row.
  - **Process gap to record in that doc:** lever 3 (a GDN out-proj prefetch in `gemv.hpp`) got only 1 of
