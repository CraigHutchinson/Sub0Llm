...
| 2026-09-08 | Claude Code | WP5a: real Qwen4 tokenizer (byte-level BPE, real `vocab.json`/`merges.txt`, gated against the real reference) | `feature/wp5a-qwen-tokenizer` | ALL NEW except two additive lines: `include/sub0/qwen_tokenizer.hpp`, `include/sub0/qwen_unicode.hpp`, `include/sub0/qwen_unicode_tables.hpp`, `src/qwen_tokenizer.cpp`, `tests/qwen_tokenizer_tests.cpp`, `tests/fixtures/qwen_tokenizer/`, `scripts/qwen_tokenizer_{tables,fixture}.py`, `docs/QWEN_TOKENIZER.md`; plus one source line in `CMakeLists.txt` and one in `tests/CMakeLists.txt`, and one `.gitignore` entry. Existing `tokenizer.hpp`/`tokenizer.cpp`/`casing.hpp` deliberately NOT touched. | done | DONE, ready to merge. 3041 assertions passing; 1936 fixture rows all produced BY the real tokenizer. Default build unaffected (235 cases / 118,416 assertions identical with WP5a removed from the build). Headline finding: `AutoTokenizer` is the WRONG oracle for this model — `tokenizer_config.json`'s stale `tokenizer_class` sends it to transformers' `Qwen2Tokenizer`, which rebuilds the pipeline with an older `\p{M}`-less regex and mis-tokenizes every Indic/Thai/pointed-Hebrew/vocalised-Arabic prompt. See `docs/QWEN_TOKENIZER.md` §2.1. Zero file overlap with WP5b, as planned. |
