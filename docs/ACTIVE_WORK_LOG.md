...
| 2026-09-22 | Sonnet 5 subagent C (isolated worktree) | O5 phase 2a: streaming native-quant unpack kernels (Q8_0/Q4_K/Q5_K/Q6_K), gated at >= 10 GB/s of compressed bytes per thread; bench fixed to compare against the real `gemv::axpy` | agent worktree | `include/sub0/backbone_quant_dot.hpp`, `tests/backbone_quant_dot_tests.cpp`, `benchmarks/backbone_quant_dot_bench.cpp`, `docs/BACKBONE_NATIVE_QUANT.md`. NO engine wiring (phase 2b waits for O4, since it touches `decode.cpp`) | done, merged | Merged 2026-09-22. Re-verified: gates exact (3,891/10; 141,609/257; 145,500/267), throughput reproduced, and the kernels are 1.21-2.17x faster per row than `gemv::axpy` at 8 threads. Phase 2b (engine wiring) NOT started and is the next real step. | Asked to commit WIP on its worktree branch and stop; see the pause note below. |
