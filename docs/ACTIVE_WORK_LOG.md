...
| 2026-09-23 | Claude Code (+ Sonnet 5 subagent) | O5 phase 2b-2b: recover the GDN roles in the S0B1 sidecar (in-proj rows permuted at write; out-proj input permutation applied to the activation), then wire every remaining sidecar role into decode (GR, QSA, MoE shared expert, GDN) with caller-owned activation scratch, then the controlled throughput gate | agent worktree | `tools/sub0llm-transplant.cpp`, `include/sub0/backbone_quant.hpp`, `src/backends/cpu/decode.cpp`, `include/sub0/*_math.hpp` call sites, tests, `docs/BACKBONE_NATIVE_QUANT.md` | active | GDN in/out projections are 49.7% of backbone bytes per token and are excluded from today's sidecar. Codex's 2b-2a TokEmb/head wiring stays as is. Will run real-artifact timing: Intel tracks please avoid hardware timing meanwhile. |
