...
| B24 | P1 (planned) | Backbone precision reduction — BF16 storage first, model-native quantized backbone second | Plan only, `docs/BACKBONE_PRECISION.md`; bandwidth argument validated numerically (~281→~140→~23-48 ms/token at F32→BF16→native-quant, 70 GB/s DRAM); prefill confirmed currently just as bandwidth-bound as decode (sequential `forward_one`, not batched) | L | User-directed priority; Phase 1 scoped in full, Phase 2 scoped at design-fork level only |
