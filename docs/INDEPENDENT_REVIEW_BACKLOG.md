...
| B25 | P0 (in-flight) | Explicit pipelined overlapped I/O for `ParallelExperts`' MoE resolve, replacing reactive `mmap` faults | Computed this session: if the 10 requested decode threads achieved full I/O concurrency, decode floors near ~847 ms/token vs. the current measured ~5.48 s/token (~6.5x gap) — B21's own still-open concurrency-ceiling finding is the largest identified cause. Implements a scoped, Sub0Llm-local version of the fix Sub0MemPage's own design already specifies, pipelined (a completed expert's compute starts immediately rather than waiting for the slowest of the batch). Correctness gate is bit-exact (pure I/O-mechanism swap). Also considering CPU-cache prefetch hints in the dequant/transpose compute path as a secondary lever. | L | Dispatched in direct response to a user-set session goal ("token-generation near CPU theoretical performance"); see `docs/ACTIVE_WORK_LOG.md` for the live row |
