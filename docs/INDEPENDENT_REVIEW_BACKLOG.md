...
| B35 | Done, merged `341e37b`, independently reverified — **first real throughput win of this thread (2.4x), but it BUYS speed with quality** | The bigger llama.cpp-matching lever: quantized-activation × quantized-weight integer SIMD dot products (quantize the activation vector once per token into a block format, e.g. `q8_K`-equivalent, then dot it against the RESIDENT quantized weight bytes directly via hand-written AVX2 intrinsics — never materializing a float weight array at all) | Confirmed via direct code reading (`ggml_vec_dot_q5_K_q8_K` et al. in `llama.cpp-qwen4exp`'s `ggml-cpu/arch/x86/quants.c`) that this — not merely "avoid a dequant round-trip" (B31's lever) — is llama.cpp's real core technique, and it is genuinely different from anything B24 Phase 2's "2a vs 2b" framing considered (that framing only compared resident-float vs. dequant-to-float-per-read; this is a third option, dequant-never, both operands stay quantized integers through the whole dot product). Real potential: this is very plausibly the single largest remaining lever toward closing B32's ~4-6x gap, since it attacks BOTH the bandwidth problem (quantized bytes, not float, cross the memory bus) AND the compute problem (integer SIMD, not scalar float) at once. | L | Deliberately NOT started now — genuinely new kernel family (does not fit the current `Node`/`WP`-templated FLOAT-kernel architecture as-is), real correctness risk (a new quantized-activation representation threaded through the whole per-token forward pass), needs its own design pass before implementation, same reasoning this session applied to B31's original cache-tiling scope. Revisit once B34 lands with real numbers — B34 is lower-risk and tells us how much of the gap is "scalar vs SIMD" alone before committing to the bigger, riskier rewrite  **DESIGN PASS DONE 2026-09-21 — see `docs/MOE_QUANT_DOT.md`; it REFRAMES this item.** Two findings. (1) **B35 belongs on the MoE expert path, not the backbone**, contrary to this row's own original framing: the backbone is NOT quantized-resident here (transplant dequantizes it offline to F32/BF16/FP8 per `PARAM_DTYPE`), so applying the technique there needs a new resident format AND a transplant-format change first — whereas the routed experts are ALREADY resident as native `IQ1_S`/`IQ2_XXS`/`IQ4_NL` GGUF bytes in the `.moeq` sidecar, needing no offline change at all; and the MoE path is the dominant cost (~2.6-2.9s of ~3.38 s/token vs the backbone's ~300-350ms). (2) **B24 Phase 2's "2a beats 2b by 5-30x" verdict does not transfer to the MoE path** — that measurement was run at the backbone's DENSE access density, where a resident buffer is re-read every token and the dequant cost amortizes; on the decode MoE path `internal.hpp`'s own `MOE_DECODE_SLOTS` comment establishes the resolve-pool hit rate is **provably zero**, so every dequantized plane is written, read exactly once, and discarded — there is nothing to amortize, and the materialization is pure overhead. Real traffic arithmetic from this sidecar's own census: **39.05 MiB/resolve today (1.55 encoded read + 18.75 dequant write + 18.75 FFN read-back) → 1.55 MiB fused = 25.3x less**; per token 18.30 GiB → 0.72 GiB, a pure-bandwidth term of ~655ms → ~26ms at this host's measured ~30 GB/s. It also replaces this engine's SCALAR `gguf::to_f32` decode with inline integer SIMD, attacking the compute term B27 measured as dominant — the same double attack (bandwidth AND compute) B32 identified as llama.cpp's real ~4-6x advantage, and the only remaining identified lever that is not a pure compute-side change on a bandwidth-bound workload (which B28/B33/B34 each independently showed does not help). All three needed formats have real AVX2 reference kernels in the local llama.cpp checkout to read as technique references. NOT bit-exact by nature (integer dot vs dequantize-then-f32-dot) — gate it like B24 gated BF16, and settle the `forward`/`forward_one` parity question (fuse both paths, or scope the toggle) BEFORE writing code.  **BUILT 2026-09-21 — and it is the first REAL decode win this thread has produced. Measured on the real 48-layer artifact, interleaved A/B, no competing process on the host: default `3.746 / 3.613 / 3.563 / 3.593 / 3.795` s/token (mean ~3.66) vs fused `1.858 / 1.771 / 1.576 / 1.436 / 1.460 / 1.513 / 1.574` s/token (steady state ~1.51, the first two samples still warming) — a ~2.4x decode speedup.** That moves B32's llama.cpp gap from ~4-6x to ~1.6-2.6x (llama.cpp 0.59-0.94 s/token on the same host/model). **What was built** (`include/sub0/moe_quant_dot.hpp`, new, 380 lines): an int8-quantized activation (`ActBlocks`, one scale + one sum-of-quants per 32 elements), three per-format block unpackers (`Iq1SPlane`/`Iq2XxsPlane`/`Iq4NlPlane`, each ~20 lines — the ONLY thing that differs per format), and ONE shared `detail::gemv` that owns the row walk, the integer multiply-accumulate and the scale fold for all three, with `Plane::kHasDelta` compiling IQ1_S's extra term away for the other two. Wired into decode ONLY (`src/backends/cpu/decode.cpp`'s MoE lambda), behind `--moe-quant-dot` → `constexpr bool MOE_QUANT_DOT`, default false. **No raw intrinsics, and that is a measurement not a preference**: B34/B38's float reductions needed coaxing because Clang will not reassociate FP addition; integer addition has no such barrier, so the portable 32-element int8 dot auto-vectorizes to `vpmovsxbw`+`vpmaddwd` (IQ2_XXS, IQ4_NL) or `vpmovsxbd`+`vpmulld`+`vphaddd` (IQ1_S) with no scalar fallback anywhere — verified in generated assembly, and `#pragma clang loop vectorize(enable)` was tried and changed neither, so it is not carried. **All three block layouts AGREED between `gguf.hpp` and ggml** (`ggml-common.h`'s struct definitions and `ggml-quants.c`'s `dequantize_row_iq*` scalar references, cross-read field by field): IQ1_S 2+32+16=50/256, IQ2_XXS 2+64=66/256, IQ4_NL 2+16=18/32, including IQ4_NL's non-obvious "byte j holds element j and element j+16" nibble split and IQ1_S's `qh` field packing. No disagreement to report. **Deliberate divergence from llama.cpp, and why**: it quantizes the activation into `block_q8_K` (one scale per 256); this engine cannot, because d_ff = 640 means down-projection row j starts at element j*640 — a HALF-block offset inside a 256-element super-block for every odd j. All three formats are internally 32-element-structured, and 2560 % 32 == 640 % 32 == 0, so GROUP=32 is the only granularity at which this engine's real shapes and every format align. It is also strictly more accurate than per-256. **Correctness, isolated before end-to-end (the one new error source, measured on its own first)**: with an activation constructed to quantize LOSSLESSLY (integers, a ±127 planted per group ⇒ scale exactly 1.0f), the fused dot agrees with `gguf::to_f32`'s own dequantized dot to **2.0e-8 / 8.7e-8 / 4.2e-7** (IQ1_S / IQ2_XXS / IQ4_NL) — float rounding only, i.e. **the weight side adds no error at all and all three unpackers are decoding the same values the existing decoder does**. The int8 ACTIVATION alone, on a Gaussian row, costs **0.48% / 0.94% / 3.3%** relative on a single 256-term dot; a whole fused expert FFN lands at **~0.9-1.1% L2-relative** against the dequantize path. **End-to-end on the real artifact**: `forward` (unfused, op_moe) vs `forward_one` (fused) gives **L2-relative logit diff 0.2938, argmax agreement 4/6** — between this project's own precedents (BF16 ~0.199 at 5/6, FP8 ~0.43), and NOT materially worse than FP8, so not a stopper; stated honestly rather than framed as small, and it is entirely B35's own contribution since the same build's default arm reproduces parity of exactly 0 with argmax 6/6. **Parity, decided up front per the design brief**: only `forward_one` is fused — `op_moe`'s batched path keeps the f32 resolve because it has a real cross-row cache hit rate and is precisely B24 Phase 2's legitimately-winning case — so `forward`/`forward_one` bit-exactness is deliberately given up IN THE FUSED BUILD ONLY. `tests/engine_tests.cpp`'s parity gate is now `(USE_MOE_QUANT && MOE_QUANT_DOT) ? 5e-2 : 1e-3`, with the reason written at the gate. **Default-build regression**: `sub0_tests` reproduces decode hash `816c4a54ad49b8cf` exactly at the `d196check` config (28,969,623 assertions / 147 cases, all green); `sub0_frontend_tests` 122,385 / 256 vs `main`'s 121,233 / 252 — +4 cases, +1,152 assertions, all of them the new B35 cases and nothing else (no non-test file the frontend links changed). The real-axes builds' `forward()` row statistics are byte-identical between the two arms, confirming the batched path is untouched. **One incidental finding worth recording**: the real-48-layer CLI recipe quoted in this session's own handover is INCOMPLETE — it omits `--rope-theta 10000000 --gdn-full-attn-stride 4 --gdn-key-head-dim 128 --gdn-value-head-dim 128 --hc-count 4 --hc-lowrank 320 --qsa-indexer-n-heads 4 --qsa-indexer-kv-heads 1 --qsa-indexer-head-dim 128 --qsa-indexer-budget 2048 --qsa-indexer-compress-ratio 4 --tie-embeddings 0` and wrongly includes `--loop-middle-layers 4`, producing `PARAM_FLOATS 2570717696 / NUM_PARAMS 626` and a rejected artifact instead of the correct `4915107200 / 1074`. The working recipe was recovered by differencing `out/build/wp5c_full48/generated/sub0_corpus.hpp` against a fresh configure, and is recorded in full in `docs/MOE_QUANT_DOT.md`'s build section so the next session does not repeat the reconstruction. **Next lever, named not attempted**: B27's own accounting said the resolve is compute-dominated; at ~1.47 s/token the fused path has removed 25x of the DRAM traffic, so what remains is the unpack-plus-MAC itself. IQ1_S is 47% of all planes and is the one format whose dot lands on the WIDER `vpmulld` shape rather than `vpmaddwd`; narrowing it (or hoisting the four grid qword loads so the weight bytes reach the dot through memory the way the other two do) is a bounded, measurable next step. |
...
| B36 | Done, merged, independently reverified | Integrate B25's pipelined I/O work into `main` as a real `--moe-io-mode reactive\|pipelined` build option (default reactive, today's behavior) instead of leaving it stranded on an unmerged local branch | **Genuine re-targeting done, not a replay of the old diff.** B25's branch predates B31; B31 replaced decode's `ExpertCache`+`dequantize_expert`+`transpose_out_in` two-step resolve with `moeq::ExpertCacheSource`/`moe::expert_ffn_row_source` (dequantize straight into GGUF SOURCE order, no transpose). B25's own `moeq::ExpertCache::resolve_from_bytes` (built against the OLD transposed-pool shape) no longer applied at all — re-derived the seam from scratch against `ExpertCacheSource`: added a NEW `ExpertCacheSource::resolve_from_bytes(store, layer, expert, raw_gate, raw_up, raw_down)` (`include/sub0/moe_quant.hpp`) that calls the SAME `dequantize_expert_source` the reactive path calls, just fed caller-supplied byte spans (moeio::PlaneIo's read destination) instead of `store.raw(d)`'s mmap span. `include/sub0/moe_io.hpp` (`moeio::PlaneIo`/`Request`) carried over UNCHANGED -- its mechanism (batched `FILE_FLAG_OVERLAPPED` reads through one shared I/O completion port) never depended on the resolve pool's shape at all, only on "caller hands me an absolute offset + byte count + destination buffer", so nothing there needed re-deriving. **Second reconciliation, not anticipated at filing time: B29 (also already merged) pinned `MOE_DECODE_THREADS` to 1** (fewer threads measured faster once B27 established this host's I/O is not the bottleneck), which invalidated B25's OWN premise that `g_moe_decode[k]`'s per-THREAD io_buf (one thread per selected expert) was the natural staging-buffer shape -- with one thread, all `EXPERTS_PER_TOK` experts' bytes must be staged concurrently regardless of thread count. Replaced with a new `MoeIoStage` (`src/backends/cpu/decode.cpp`) holding `MOE_IO_MAX_INFLIGHT` (= EXPERTS_PER_TOK * moeq::PerExpert = 30 at the real axes) staging buffers, indexed by SELECTION ORDER (`k`, the router's own top-k slot) rather than by thread -- `ParallelExperts::prefetch(idx, n)` (moe_math.hpp's own hook, detected via `requires` and now living in the SHARED `forward_row_via_run_ex` core so both `forward_row_via_run` and B31's own `_ex` entry point get it for free) issues all `n` experts' plane reads into this stage before ANY resolve starts; the single decode thread then walks k serially, `wait()`-ing only for the k it is about to consume while the OS keeps completing the rest in the background -- overlapping ONE thread's compute with ITS OWN still-in-flight later reads, not several threads' reads overlapping each other (the shape B25 originally targeted). `--moe-io-mode reactive\|pipelined` -> `constexpr bool MOE_IO_PIPELINED` (default `false`/reactive, `tools/configurator.cpp`, one CLI option + one `cos <<` emission + one configure-time cross-check that pipelined requires `--moe-quant-experts 1`, all localized near `--moe-quant-experts`/`--prec-param` — 35 lines total, nothing else in that file touched). `g_moe_decode_io` (the explicit-I/O file handle) opened as a SECOND, independent handle onto the same `.moeq` sidecar only under `MOE_IO_PIPELINED`, right after `g_moe_quant.open()` in `load_moe_quant_sidecar` (`src/backends/cpu/backend.cpp`) — the default build never opens it at all. | M | **Regression-safety (default/reactive build unaffected)**: `sub0_frontend_tests` 120,923/245 — EXACT match to the documented pre-B36 `main` baseline (this suite's own MoE-exercising cases, `moe_quant_tests.cpp`/`moe_qwen4_fixture_tests.cpp`, run at synthetic axes with `MOE_IO_PIPELINED` at its default-false value, so this is the real regression gate for the reactive arm). **Bit-exactness at BOTH settings, on the real 48-layer BF16 artifact** (`D:\ModelWeights\Sub0Llm-Qwen4-full48-bf16\qwen4_full48_q_bf16.bin`, built through the real `sub0llm-configure` at the production axes, `--moe-io-mode reactive` and `--moe-io-mode pipelined` in two separate build directories): `sub0llm-qwen4-forward --tokens 3` gives `forward`/`forward_one` parity **exactly 0** at BOTH settings, and the per-row logit statistics (mean/rms/range/argmax, 3 rows) printed BYTE-IDENTICAL between the reactive and pipelined builds (row 0 argmax 487, row 1 argmax 198, row 2 argmax 220, matching means/rms to all printed digits at both settings) — the pipelined path is a pure I/O-mechanism swap, verified, not merely argued. **Throughput**: measured on this host WHILE B37/B38 were independently dispatched and building/benchmarking in parallel on the SAME host (`docs/ACTIVE_WORK_LOG.md`'s own B36/B37/B38 dispatch entry) — real, confirmed external contention, not an artifact: raw per-run wall-clock varied 2x run-to-run within the SAME build (reactive: 9.75-19.52s/3 tokens; pipelined: 9.79-21.78s/3 tokens) with no stable ordering either direction. The CLEAN (least-contended) sample cluster from each arm lands close together — reactive's fastest runs 9.75s/3.63s ≈ 3.25-3.63 s/token, pipelined's fastest three 9.79-9.98s ≈ 3.27-3.33 s/token — consistent with B25/B27's own "no measurable win, I/O is not the bottleneck on this warm-page-cache host" finding, but this session's own measurement is NOT a clean reproduction of B25's precise "~1.4% worse" figure: today's contention from concurrent sibling work makes a tight A/B impossible to certify from this pass alone. Reported honestly rather than forcing a number. | Kept default-off exactly as filed; the pipelined arm is a real, independently-buildable, bit-exact capability for a machine/session state where the sidecar is NOT page-cache-resident (cold boot, memory pressure, a host too small to cache 37 GiB at all) — see the CLI option's own help text and `include/sub0/moe_io.hpp`'s header comment for the documented, situational recommendation. Files touched: `include/sub0/moe_io.hpp` (new, carried over from `feature/b25-explicit-io-resolve` with header-comment updates reflecting B31/B29's own since-merged changes), `include/sub0/moe_quant.hpp` (`Store::max_desc_bytes()`, `ExpertCacheSource::resolve_from_bytes`), `include/sub0/moe_math.hpp` (the `prefetch` hook in `forward_row_via_run_ex`), `src/backends/cpu/internal.hpp` (`MOE_IO_MAX_INFLIGHT`, `g_moe_decode_io` extern), `src/backends/cpu/decode.cpp` (`MoeIoStage`, `ParallelExperts::prefetch`, the pipelined branch in `forward_one`'s MoE call site), `src/backends/cpu/backend.cpp` (`g_moe_decode_io` definition + conditional open), `tools/configurator.cpp` (`--moe-io-mode`, one new flag, localized). Zero overlap with B37's (`fp8.hpp`/`param_store.hpp`/`model_file.hpp`/`sub0llm-transplant.cpp`) or B38's (`simd_reduce.hpp`/the four `*_math.hpp` REDUCTION bodies -- this package's own `moe_math.hpp` touch is a different function, `forward_row_via_run_ex`'s prefetch hook, not `expert_ffn_row`/`expert_ffn_row_source`'s summation loops) files. **Independently reverified before merging**: read `resolve_from_bytes`, the `prefetch`-detection hook, and `decode.cpp`'s pipelined branch in full myself — the bit-exactness argument holds by construction (same dequant function, same bytes). Rebuilt `sub0_frontend_tests` myself: 120,923/245, exact match. Built BOTH real 48-layer configs (reactive/pipelined) myself and dumped `forward_one`'s raw logits from each — the two dump files are BYTE-IDENTICAL (`cmp` confirms), independently proving bit-exactness beyond the tool's own parity check. My own quick interleaved timing (2 runs each): reactive ~3.03 s/token vs pipelined ~3.14 s/token — pipelined slightly slower, same direction as the documented finding. Merged (conflict in `docs/ACTIVE_WORK_LOG.md`/`INDEPENDENT_REVIEW_BACKLOG.md` against B37's own already-merged entries, resolved by keeping both packages' log entries in full); hit the same stale-generated-config issue (`d196check`'s header predated `MOE_IO_PIPELINED`) — rebuilt `sub0llm-configure`, regenerated, full rebuild + both suites green (28,969,623/147 + 121,211/251), decode hash unchanged (`816c4a54ad49b8cf`). Pushed (`d59fe30`). Deleted the now-superseded `feature/b25-explicit-io-resolve` branch and its worktree. |
| B38 | Done, merged, independently reverified | Integrate B34's SIMD-reduce work into `main` as a real `--simd-reduce` build option, default `false` (today's scalar behavior, bit-exact) | Same user directive as B36/B37. Prefer a shared, templated kernel body over duplicating each kernel twice, to avoid drift risk between the two arms. | M | **Design**: new `constexpr bool USE_SIMD_REDUCE` (`--simd-reduce`, default 0) emitted by `tools/configurator.cpp` next to `USE_TERNARY` (one new option, nothing else touched in that shared file). `include/sub0/simd_reduce.hpp` carries B34's own `dot()`/`dot_seq()`/`sumsq()`/`sum()` UNCHANGED, plus three new `*_choice<UseSimd>()` dispatchers (`dot_choice`/`sumsq_choice`/`sum_choice`) -- the ONE seam every kernel's reduction call site goes through, `if constexpr`-selecting between B34's SIMD primitive and a scalar loop that is byte-for-byte the ORIGINAL pre-B34 accumulation shape. Every one of the four kernel files' surrounding logic (loop bounds, tiling, SiLU gating, softmax) stays written once; only the innermost reduction call differs. Shared/templated shape achieved everywhere: `gdn::forward`/`gdn::backward`, `gr::hc_norm`, `qsa::rms_norm_row`/`indexer_select_row`/`attn_project_row`/`attn_row`/`forward`, and `moe::forward_row_via_run_ex` (+ its thin wrappers `forward_row_via_run`/`forward_row_via`/`forward_row`/`forward`/`forward_via`) all gained a leading `bool UseSimd = false` template parameter, threaded explicitly from the generated `USE_SIMD_REDUCE` at every engine call site in `src/backends/cpu/{backend,decode}.cpp` (CUDA's own CPU-reference call in `backend.cu` and every fixture test keep the default). No duplicated-body fallback was needed for any of the four files -- the dispatcher pattern covered every real call site cleanly. **`expert_ffn_row_source`'s three call sites are the one deliberate exception**: they use `simd::dot_seq` UNCONDITIONALLY, completely outside the `UseSimd` template parameter (moe_math.hpp has no `UseSimd` parameter on that function at all) -- B34's own found-and-fixed correctness constraint (B31's forward()/forward_one() bit-exactness pairing with `expert_ffn_row`) applies regardless of the build's reduction-strategy toggle, and both this backlog entry and a comment at the call site say so explicitly. **Verification**: (1) default-arm regression gate -- `d196check`'s own historical corpora (`data/gsm8k.txt` etc.) are git-LFS/untracked and were not present in this isolated worktree, so the literal `816c4a54ad49b8cf` hash could not be reproduced directly; instead verified via two INDEPENDENT, arguably stronger routes: (a) `sub0_frontend_tests` -- which DOES exercise all four kernel files at the real Qwen4 axes via `moe_quant_tests.cpp`/`moe_qwen4_fixture_tests.cpp`/`gdn_qwen4_fixture_tests.cpp`/`qsa_qwen4_fixture_tests.cpp`/`gated_residual_qwen4_fixture_tests.cpp`, all fixture-driven and needing no external corpus -- reproduced the EXACT pre-existing count, 120,923 assertions/245 cases, before adding any new test; (b) a synthetic `--vocab-exact` config (no corpus needed) with GDN+MoE+QSA+GR ALL explicitly enabled, built and run through the real compiled engine (`sub0_core`/`sub0_tests`): `arch_identity_tests.cpp`'s forward/decode fingerprint test reproduced the IDENTICAL hash pair (forward `0xdbf452a2d188e5b2`, decode `0x7510a105755db578`) across a from-scratch reconfigure+rebuild at `USE_SIMD_REDUCE=false`, and `engine_tests.cpp`'s `forward_one`-vs-`forward` parity check passed (worst rel diff 1.63e-7, well inside its own established 1e-3 tolerance). (2) ON-arm (`--simd-reduce 1`) at the SAME synthetic axes: builds clean, same parity check passes (1.60e-7, same order), and the fingerprint hashes CHANGE as expected (forward `0xbfeb0ff9602e504`, decode `0xbc4a564ef53a97b4`) -- proving the flag has real, live effect through the actual compiled engine rather than being dead code, while staying deterministic (bit-identical across repeated runs at the same setting). A new permanent test, `moe_quant_tests.cpp`'s "moeq (B38)" case, instantiates `moe::forward_row_via_run_ex<true>` directly against the same real sidecar-format bytes the B31/B34 gate test uses and requires: both arms finite, the two arms' output close (loose 1e-3 tolerance, not bit-exact -- reassociation is expected to move the last bits), AND `expert_ffn_row_source`'s own output IDENTICAL between the two runs -- proving the flag stays scoped to only the shared-expert gate logit and never leaks into the routed-expert path. `sub0_frontend_tests` with this new case: 120,945/246, all green. (3) **Real-artifact throughput spot-check NOT reproduced this session**: the real 48-layer BF16 transplanted artifact (and its MoE quantized sidecar, ~55 GiB combined) that B34's own ~20% regression number and this task's requested spot-check both depend on was not present anywhere in this isolated worktree or its accessible filesystem (searched; absent) -- reported honestly rather than fabricated or assumed. The architecture is unchanged from B34's own already-independently-reconfirmed implementation (same `simd_reduce.hpp` primitive, same call sites, same `dot_seq` exclusion), so B34's own recorded ~20% decode slowdown (3.09s -> 3.72s mean s/token) remains the standing, citable number for this tradeoff pending a future session with access to that artifact re-measuring post-merge. Flagged explicitly per this task's own honesty requirement rather than silently omitted. `tools/configurator.cpp`: touched ONLY to add the one `--simd-reduce` option (variable declaration, CLI option with the honest-tradeoff comment, header emission) -- nothing else in that shared file was restructured. **Independently reverified before merging**: read `dot_choice`/`sumsq_choice`/`sum_choice` in full myself — confirmed the `UseSimd=false` arm of each is a byte-for-byte copy of the original scalar reduction, so default-off bit-exactness is provable from the code shape, not just tested. Rebuilt the B38-specific test and the full frontend suite myself at this branch's own `native` config: 56/2 and 120,945/246, exact match. Verified the auto-merged `forward_row_via_run_ex` signature (`template <bool UseSimd = false, class WP, ...>`) sits correctly alongside B36's own already-merged prefetch hook in the same function body. Resolved a real conflict at `decode.cpp`'s MoE call site (B36's pipelined-I/O lambda vs. B38's `<USE_SIMD_REDUCE>` template arg on the same `forward_row_via_run_ex` call) by combining both — the lambda body is entirely B36's, only the outer call gained the template argument. Attempted to reproduce the real 48-layer artifact check directly (matching B36/B37's own pattern) but could not reconstruct the exact real-axes CLI recipe (several non-obvious flags: `--rotary-dim 64` distinct from `--head-dim 256`, GDN loop-middle semantics) within a reasonable time budget — a reconstruction-effort limitation, not a code defect, superseded by the by-construction correctness argument plus the exact-match suite results. Merged (real conflict, resolved as above); pushed. |
| B39 | Done, measured NET REGRESSION, NOT merged (kept on `feature/b39-constexpr-shape`) | Apply AGENTS.md §2 (bake the known shape as `constexpr`) to B35's fused MoE kernel: template `gemv`/`expert_ffn_row_quant`/`ActBlocks` on their dimensions instead of passing compile-time-constant trip counts through a runtime interface | User's own directive: *"we have configurable constexpr/compile-time decisive support we can use to make the code specific and avoid runtime/user configurability where the model is actively only one shape."* B35 passed `n_rows`/`row_elems` as runtime `int` (always 640/2560 and 2560/640), divided `row_elems / GROUP` at runtime (folds to 80/20), sized `ActBlocks` via `std::vector` at runtime (widths always D_MODEL/D_FF — also an AGENTS.md §1 heap-in-a-per-token-path miss), and ran geometry checks on constants. `layout.hpp:482` already had `inline constexpr moe::Dims MOE_DIMS{...}`, so the constants were right there. | M | **The `moe::Dims` C++23 NTTP works cleanly** (`template <moe::Dims D>`), and numerics stayed bit-identical throughout (`max\|forward-forward_one\| = 4.93666` at row 5 / vocab 143211, same value and location as B35). **But it measured SLOWER, interleaved, zero contention**: before the alignment fix below, B35 1.535 vs B39 1.662 s/token (−8.3%); after, B35 1.470 vs B39 1.518 (−3.3%). **A real bug this change introduced, found while chasing that gap**: `std::array<std::int8_t, N>` has natural alignment 1, so embedding it in `MoeDecodeThread`/`g_moe_act_q` let it land at any byte offset and `dot_group`'s vector loads ran unaligned — the `std::vector` it replaced got heap alignment for free. `alignas(64)` recovered ~60% of the gap. **The remaining ~3% is not the templating**: generated `sub0_core.dll` differs by 0.02% between arms (2,834,432 vs 2,834,944 B), so compile-time trip counts changed neither unrolling nor vectorization — Clang was ALREADY constant-folding the dims through inlining from the `constexpr MOE_DIMS` call site. The templates changed the spelling, not the optimizer's information; what remains is the storage change (inline array vs pointer-to-heap altering `MoeDecodeThread`'s size and locality). **Verdict: not merged.** §2's own stated rationale is "the unused branch compiles away entirely" — there was no runtime branch to eliminate here, so the rule's purpose was already satisfied by inlining, and paying a measured 3% on the hot path to restate it in the type system is the wrong trade. The `alignas` lesson is the durable finding: dropping `std::vector` for `std::array` to satisfy §1 silently forfeits heap alignment, and in a SIMD kernel that is worth ~5%. |
| B40 | Done 2026-09-22 — **CORRECTS B21 and B29** | Why did decode's OpenMP teams never scale? | `ensure_thread_built()` pinned the MAIN thread to one logical CPU before any OpenMP region, and LLVM libomp places every team inside its master's affinity mask — so every decode team (B20's per-expert fan-out, O2's threaded GEMV) ran time-sliced on ONE core. Reproduced in isolation (`sub0llm-bench-gemv`, GDN in_qkv at 8 threads: master pinned to one CPU 15.7 GB/s, slower than 1 thread; master pinned to the P-core set 51-62 GB/s; initializing OpenMP first does NOT help). Training never showed it because train_batch's workers re-pin themselves in ensure_thread_built; decode's teams never call it. **B29's "one thread is fastest, DRAM contention" and B21's "concurrency ceiling" were this artefact**: their signature (each expert starting exactly as the previous ends, more threads slower) is single-core time-slicing. Fix: the initial thread takes the P-core SET (`cpu_affinity.hpp` `pin_current_thread_p_set`). Re-measured: routed experts 185 → 39 ms at 10 threads, decode 0.474 → 0.314 s/token with O2's threaded GEMV, L2 bit-identical, suites and decode hash unchanged. | S | Lesson for the process: a parallel region that does not scale is a placement question before it is a bandwidth question — check the master's affinity mask first (docs/OPTIMIZATION_PROCESS.md). |
