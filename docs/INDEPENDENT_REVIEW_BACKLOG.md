...
| B31 | Not started, scoped | Fuse `moeq::ExpertCache`'s dequant→transpose→FFN resolve pipeline to cut redundant DRAM round-trips (currently ~3 full read/write passes over each expert's ~19.66 MB per-plane-set, per B30's byte accounting) | Not yet attempted — named by B30's synthesis as the one remaining structural lever now that I/O (B27), the IQ2_XXS defect (B28), and thread concurrency (B29) are all resolved. Real potential: closing some fraction of the gap between the current ~44-52% single-thread bandwidth utilization and the theoretical ~100% ceiling (~1.7-2.0 s/token if fully captured, vs. today's ~3.57-3.67 s/token) — a meaningful (~40-50%) further reduction, not a multiple-x one. | L | Genuine redesign, not a same-pattern fix: touches `dequantize_iq*`'s output shape, `transplant::transpose_out_in`, and `moe::expert_ffn_row`'s input contract together, so it needs its own design pass (can dequant write directly in transposed order, or can the FFN read the untransposed layout directly, eliminating one or both intermediate DRAM round-trips?) and its own correctness fixture, not an opportunistic edit. Scope deliberately, the way §2c/§2d of `docs/CPU_PERF_BACKLOG.md` scoped `qsa::linear_row` and the pre-fix `dequantize_iq2_xxs` — do not fold into an unrelated task |
