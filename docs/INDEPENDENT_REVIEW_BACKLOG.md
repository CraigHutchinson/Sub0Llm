...
| B31 | Dispatched (active) | Fuse `moeq::ExpertCache`'s dequant→transpose→FFN resolve pipeline to cut redundant DRAM round-trips (currently ~3 full read/write passes over each expert's ~19.66 MB per-plane-set, per B30's byte accounting) — **CACHE-TILED, not just fewer passes** (user's own direction, verbatim: "we should be L1/L2/L3 cache aware in our implementation as to utilise explicit cpu caches where available closer to the core instead of necessarily writing back to main system ram... part of the 'constexpr' set of optimizations our sub0llm build architecture allows us to do micro optimized work sizing") | Named by B30's synthesis as the one remaining structural lever now that I/O (B27), the IQ2_XXS defect (B28), and thread concurrency (B29) are all resolved, and directly validated as real (not speculative) by B32's llama.cpp comparison. The mechanism is more specific than "avoid a round-trip": process the resolve in TILES sized to fit a P-core's 3 MB L2 (or, if tiling across stages needs more working set than that, the 36 MB shared L3 — `docs/host-cpu-arrow-lake-hx.md`, this host is Arrow Lake-HX, 8P+16E, no SMT, P-core L2 3 MB/E-core L2 4 MB shared/4, L3 36 MB) so a tile's dequantized bytes are read back by the transpose stage, and the transposed tile's bytes read back by the FFN stage, WHILE STILL RESIDENT IN CACHE rather than having round-tripped through DRAM — this is very likely the real mechanism behind llama.cpp's own ggml kernels' advantage (B32), which are well known for exactly this kind of tiled/blocked fused compute. Tile sizes should be `consteval`/`constexpr`-derived from the real per-core cache sizes and the real per-plane dimensions (`D_FF=640`, `D_MODEL=2560` at the real Qwen4 axes), matching this project's own standing compile-time-over-runtime preference (AGENTS.md §1/§8, `[[compile-time-decisions-preferred]]`) — a baked tile size for THIS build's real shape, not a runtime-tuned or generic one. Real potential: closing some fraction of the gap between the current ~44-52% single-thread bandwidth utilization and the theoretical ~100% ceiling (~1.7-2.0 s/token if fully captured, vs. today's ~3.57-3.67 s/token) — a meaningful (~40-50%) further reduction, not a multiple-x one, though a genuinely cache-resident fused pipeline could plausibly beat the pure-bandwidth estimate by also cutting compute-side cache-miss stalls the byte-counting arithmetic doesn't capture. | L | Genuine redesign, not a same-pattern fix: touches `dequantize_iq*`'s output shape, `transplant::transpose_out_in`, and `moe::expert_ffn_row`'s input contract together, so it needs its own design pass (can dequant write directly in transposed order into a small cache-resident tile, or can the FFN read the untransposed layout directly, eliminating one or both intermediate DRAM round-trips AND keeping the working set cache-resident throughout?) and its own correctness fixture, not an opportunistic edit. Scope deliberately, the way §2c/§2d of `docs/CPU_PERF_BACKLOG.md` scoped `qsa::linear_row` and the pre-fix `dequantize_iq2_xxs` — do not fold into an unrelated task |
