...
const float* Model::forward_one(int id, int pos) {
    ...
    for (int e = 0; e < LOOP_EXEC_COUNT; ++e) {
        ...
        if constexpr (USE_MOE) {
            ...
            // B34/B38: UseSimd explicit template arg selects the shared-expert gate-logit reduction
            // strategy; see docs/INDEPENDENT_REVIEW_BACKLOG.md B38 and include/sub0/simd_reduce.hpp.
            // expert_ffn_row_source below is NEVER gated by this -- it always uses simd::dot_seq
            // regardless, to preserve B31's forward()/forward_one() bit-exactness invariant.
